...
with open('basecalling_speed.csv','r') as csvfile:
        ...
        for row in plots:
            ...
            RUBICON.append(float(row[5]))
            RUBICON_AIE.append(float(row[6]))


Causallcall = np.log10(np.array(Causallcall))
Guppy = np.log10(np.array(Guppy))
Guppy_fast = np.log10(np.array(Guppy_fast))
Bonito = np.log10(np.array(Bonito))
RUBICON = np.log10(np.array(RUBICON))
...
ax = plt.subplot(111)
plt.ylabel('Basecalling Throughput\n(log kbp/sec)', labelpad=0.1)

ind = np.arange(start = 0, stop = 25, step = 2.5)
...
plt.legend(bbox_to_anchor=(0.98, 1.2),loc='upper right', ncol=7 , \
...
plt.axvspan(22,25,color='#999999', alpha=0.5)
ax.set_xlim([-0.4, 24.2])
ax.set_ylim([0,3.5])
plt.yticks([1, 2, 3], ['10', '100', '1000'])
ax.tick_params(axis='y', which='major', pad=0.6)
ax.tick_params(axis='x', which='major', pad=0.6)

plt.savefig("basecalling_speed_aie_AMD_CRF_2"+".pdf", dpi=600, format='pdf', transparent=False, bbox_inches='tight')
