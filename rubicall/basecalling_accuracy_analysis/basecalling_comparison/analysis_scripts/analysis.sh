...
# $1: complete path of basecalled read: /mnt/batty/gagsingh/bonito_nas/bonito/data_organism/LATEST/basecalled_smallquant_07_24_2022/acinetobacter.fastq
# $set extracts organism 
# $basecaller: extracts folder name one level above

threads=20

echo "input: "$1
set=$(basename $1 | sed 's/.fastq.gz//' | sed 's/.fasta.gz//' | sed 's/.fastq//' | sed 's/.fasta//')
echo "organism is : "$set
DIR="${1%/*}"
basecaller=$(basename $DIR)
if [[ $basecaller = "dna_r9.4.1_e8_fast@v3.4" ]]; then basecaller=dna_r9.4.1_e8_fast\@v3.4
fi
basecaller=$(basename $DIR)
echo "basecaller is : "$basecaller
assembly_dir=/mnt/batty/gagsingh/Basecalling-comparison/bonito_assembly_WAHEGURU/$basecaller     # where finished assemblies will go
nanopolish_dir=nanopolish   # where Nanopolished assemblies will go
results_dir=/mnt/batty/gagsingh/Basecalling-comparison/bonito_results_WAHEGURU/$basecaller         # where the result tables will go
    # where the result tables will go
...
echo "--------------------------------------------------------------------------------"
mkdir -p "$results_dir"
minimap2 -x map-ont -t $threads -c reference_$set.fasta $1 > $read_alignment
...
echo "Run multiple replicates of Racon: "
echo "--------------------------------------------------------------------------------"
...
for i in {0..9}; do
    ...
    echo "[DEBUG::ANALYSIS.SH]sample_reads: "$sample_reads
    echo "[DEBUG::ANALYSIS.SH]sample_assembly: "$sample_assembly
    echo "[DEBUG::ANALYSIS.SH]sample_reference: "$sample_reference
    echo "[DEBUG::ANALYSIS.SH]trimmed_reads: "$trimmed_reads
    echo "[DEBUG::ANALYSIS.SH]assembly_reads: "$assembly_reads
    ...
    "$scripts_dir"/rotate_reference.py "$scripts_dir"/reference_$set.fasta $i > $sample_reference
...
rm $trimmed_reads
echo "Run final Rebaler: "
echo "--------------------------------------------------------------------------------"

rebaler -t $threads reference_$set.fasta $assembly_reads > $final_assembly
rm $assembly_reads
...
echo "final_assembly: "$final_assembly
...
minimap2 -x asm5 -t $threads -c reference_$set.fasta $assembly_pieces > $assembly_alignment
...
nucmer --prefix="$prefix" reference_$set.fasta $final_assembly
...
# minimap2 -x asm5 -t $threads -c reference_$set.fasta $nanopolish_pieces > $nanopolish_alignment
...
# nucmer --prefix="$prefix" reference_$set.fasta $nanopolish_assembly
# delta-filter -r -q "$prefix".delta > "$prefix".filter
...
# minimap2 -x asm5 -t $threads -c reference_$set.fasta $nanopolish_2_pieces > $nanopolish_2_alignment
...
# nucmer --prefix="$prefix"_2 reference_$set.fasta $nanopolish_2_assembly
...
# minimap2 -x asm5 -t $threads -c reference_$set.fasta $nanopolish_3_pieces > $nanopolish_3_alignment
...
# nucmer --prefix="$prefix"_3 reference_$set.fasta $nanopolish_3_assembly
...
# minimap2 -x asm5 -t $threads -c reference_$set.fasta $nanopolish_4_pieces > $nanopolish_4_alignment
...
# nucmer --prefix="$prefix"_4 reference_$set.fasta $nanopolish_4_assembly
